...
class BugNav:
	...
	def tileScoreBridge(self, ct, loc, ignoreEmpty):
		return self.tileScoreBridgeBool(ct, loc, ignoreEmpty, False)

	def tileScoreBridgeBool(self, ct, loc, ignoreEmpty, checkAllyBehind):
		if(not self.onTheMap(ct, loc)):
			return -99999
		emptyPenalty = 0 if ignoreEmpty else 3
		score = 0
	...
	def calcBestDirBridge(self, ct, currentLoc, target, lastLoc):
		...
		score1 = self.tileScoreBridge(ct, currentLoc.add(dirToTarget), False)
		score2 = self.tileScoreBridge(ct, currentLoc.add(dirToTarget.rotate_left()), False)
		score3 = self.tileScoreBridge(ct, currentLoc.add(dirToTarget.rotate_right()), False)
	...
	def MOVE_to_target_with_bridge(self, ct, loc: Position):
		...
		self.lastLocation = self.currentLocation
		self.currentLocation = ct.get_position()
		if(self.lastBridgePos == None or (self.lastTargetLocation is not None and self.lastTargetLocation != loc)):
			self.lastBridgePos = ct.get_position()
		if(self.lastTargetLocation == None or self.lastTargetLocation.distance_squared(loc) > 8 or self.bugStackIndex >= self.MAX_STACK_SIZE-10):
			self.bugStack = [None] * self.MAX_STACK_SIZE
			self.bugStackIndex = 0
			self.lastTargetLocation = loc
			...
			if(bestDir is not None and bestScore > -20):
				...
				if(bestNextDir and bestNextScore > -20):
					...
					if(self.lastBridgePos is not None and ct.is_in_vision(self.lastBridgePos) and (self.lastBridgePos.distance_squared(ct.get_position().add(bestDir)) >= 4 ) ):
						...
						if(not ct.is_in_vision(self.lastBridgePos) or ct.get_tile_building_id(self.lastBridgePos) == None):
							if(ct.can_build_bridge(self.lastBridgePos, ct.get_position().add(bestDir).add(bestNextDir))):
								ct.build_bridge(self.lastBridgePos, ct.get_position().add(bestDir).add(bestNextDir))
								self.lastBridgePos =  ct.get_position().add(bestDir).add(bestNextDir)
						
							elif(ct.can_build_bridge(self.lastBridgePos, ct.get_position().add(bestDir))):
								ct.build_bridge(self.lastBridgePos, ct.get_position().add(bestDir))
								self.lastBridgePos =  ct.get_position().add(bestDir)
							if(ct.can_move(bestDir)):
								ct.move(bestDir)
							return
		...
		if(self.RIGHT):
			...
			for i in range(8):
				if(not self.canMoveBridge(ct, ct.get_position().add(dir)) or self.tooCloseToDanger(ct, ct.get_position().add(dir)) ):
					...
					if(self.lastBridgePos is not None and ct.is_in_vision(self.lastBridgePos) and self.lastBridgePos.distance_squared(ct.get_position().add(dir).add(dir)) >= 6):
						...
						if(self.canMove(ct, ct.get_position().add(dir).add(dir)) and ct.can_build_bridge(self.lastBridgePos, ct.get_position().add(dir).add(dir))):
							ct.build_bridge(self.lastBridgePos, ct.get_position().add(dir).add(dir))
							self.lastBridgePos =  ct.get_position().add(dir).add(dir)
						
						elif(ct.can_build_bridge(self.lastBridgePos, ct.get_position().add(dir))):
							ct.build_bridge(self.lastBridgePos, ct.get_position().add(dir))
							self.lastBridgePos =  ct.get_position().add(dir)
