...
class BldRush():
    ...
    def ATTACKABLE_update(self, ct: Controller):
        ...
        for tile_pos in ct.get_nearby_tiles(12):
    ...
    def RUSH_attack(self, ct: Controller):
        ...
        if self.target_attackable == Position(-1, -1):
            if(self.explorePos == Position(-1, -1) or self.explorePos.distance_squared(ct.get_position()) < 10):
                while(True):
                    self.explorePos = Position(random.randint(self.enemy_core_pos.x - 5, self.enemy_core_pos.x + 5), random.randint(self.enemy_core_pos.y - 10, self.enemy_core_pos.y + 10))
                    if(ctx.IS_in_map(self.explorePos)):
                        break
            ctx.bugnav.MOVE_to_target(ct, self.explorePos, False, 0, 0, 2)
        for bid in ct.get_nearby_buildings():
    ...
    def RUSH_run(self, ct: Controller):
        ...
        if(self.state == "FIND_CORE"):
            if(self.openingRush):
                self.enemy_core_pos = ctx.CORE_POS
                self.state = "ATTACK"
            else:
    ...
    def RUSH_invariant_action(self, ct: Controller):
        ...
        if(self.enemy_core_pos != Position(-1, -1) and not self.openingRush):
